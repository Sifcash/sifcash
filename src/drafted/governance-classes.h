 ...
 *
 *   // network
 *   CSifcashNetwork lvl, network-type, network-status, network-error, milestone-status*
...
private:
    ...
    // Current OBJECT STATUS (see http://govman.sif.kz/index.php/Documentation_:_Status_Field)
    int nStatusID;
...
// // root node
class CSifcashNetwork : public CGovernanceObject
{
private:
    std::string strName;
    std::string strURL;
...
public:
    CSifcashNetwork(UniValue objIn)
    {
...
// // can be under: SifcashNetwork
// //   -- signature requirements : Key1(User)
// class CSifcashNetworkVariable : public CGovernanceObject
// {
// private:
