...
CPubKey CWallet::GenerateNewKey(uint32_t nAccountIndex, bool fInternal)
{
    ...
    if (IsHDEnabled()) {
        ...
        // Compressed public keys were introduced in version 0.4.0
        if (fCompressed)
...
bool CWallet::SetMinVersion(enum WalletFeature nVersion, CWalletDB* pwalletdbIn, bool fExplicit)
{
    ...
    if (fFileBacked)
    {
        ...
        if (nWalletVersion > 40000) // introduced in version 0.4.0
        ...
        LOCK2(cs_main, cs_wallet);
        {
            ...
            while (true)
            {
                ...
                if (fUseInstantSend && nValueIn > sporkManager.GetSporkValue(SPORK_5_INSTANTSEND_MAX_VALUE)*COIN) {
                    strFailReason += " " + strprintf(_("InstantSend doesn't support sending values that high yet. Transactions are currently limited to %1 SIF."), sporkManager.GetSporkValue(SPORK_5_INSTANTSEND_MAX_VALUE));
                    return false;
                }
                ...
                if (nChange > 0)
                {
                    ...
                    if (nCoinType == ONLY_DENOMINATED) {
                        ...
                        // change transaction isn't always pay-to-sifcash-address
                        CScript scriptChange;
