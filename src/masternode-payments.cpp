...
*
*   Why is this needed?
*   - In Sifcash some blocks are superblocks, which output much higher amounts of coins
...
{
    if(fLiteMode) return; // disable all Sifcash specific functionality
...
bool CMasternodeBlockPayees::IsTransactionValid(const CTransaction& txNew)
{
    ...
    BOOST_FOREACH(CMasternodePayee& payee, vecPayees) {
        if (payee.GetVoteCount() >= MNPAYMENTS_SIGNATURES_REQUIRED) {
            ...
            if (!sporkManager.IsSporkActive(SPORK_23_MN_PAYEE_VALIDATION)) {
                BOOST_FOREACH(CTxOut txout, txNew.vout) {
                    if (nMasternodePayment == txout.nValue) {
                        ...
                        return true;
                    } else if (sporkManager.GetSporkValue(SPORK_24_MN_PAYEE_VALIDATION_VAR_SUM) > 0) {
                        if (nMasternodePayment >= sporkManager.GetSporkValue(SPORK_24_MN_PAYEE_VALIDATION_VAR_SUM) &&
                            (nMasternodePayment + sporkManager.GetSporkValue(SPORK_24_MN_PAYEE_VALIDATION_VAR_SUM) >= txout.nValue &&
                            nMasternodePayment - sporkManager.GetSporkValue(SPORK_24_MN_PAYEE_VALIDATION_VAR_SUM) <= txout.nValue)) {
                                LogPrint("mnpayments", "CMasternodeBlockPayees::IsTransactionValid -- Found required payment, though expected payee '%s', amount: %f coins\n", strPayeesPossible, (float)nMasternodePayment/COIN);
                                return true;
                        }
                    }
                }
    ...
    if (!sporkManager.IsSporkActive(SPORK_25_MN_PAYEE_VALIDATION_KEEPSCOPE) &&
        sporkManager.GetSporkValue(SPORK_24_MN_PAYEE_VALIDATION_VAR_SUM) > 0) {
        BOOST_FOREACH(CTxOut txout, txNew.vout) {
            if (nMasternodePayment >= sporkManager.GetSporkValue(SPORK_24_MN_PAYEE_VALIDATION_VAR_SUM) &&
                (nMasternodePayment + sporkManager.GetSporkValue(SPORK_24_MN_PAYEE_VALIDATION_VAR_SUM) >= txout.nValue &&
                nMasternodePayment - sporkManager.GetSporkValue(SPORK_24_MN_PAYEE_VALIDATION_VAR_SUM) <= txout.nValue)) {

                LogPrint("mnpayments", "CMasternodeBlockPayees::IsTransactionValid -- Found required payment out of MN scope, though expected payee '%s', amount: %f coins\n", strPayeesPossible, (float)nMasternodePayment/COIN);
                return true;
            }
        }
    }

    LogPrintf("CMasternodeBlockPayees::IsTransactionValid -- ERROR: Missing required payment, possible payees: '%s', amount: %f SIF\n", strPayeesPossible, (float)nMasternodePayment/COIN);
    return false;
}
