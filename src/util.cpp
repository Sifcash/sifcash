...
const char * const BITCOIN_CONF_FILENAME = "sifcash.conf";
const char * const BITCOIN_PID_FILENAME = "sifcashd.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "sifcash" is a composite category enabling all Sifcash-related debug output
            if(ptrCategory->count(string("sifcash"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\SifcashCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\SifcashCore
    // Mac: ~/Library/Application Support/SifcashCore
    // Unix: ~/.sifcashcore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "SifcashCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/SifcashCore";
#else
    // Unix
    return pathRet / ".sifcashcore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty sifcash.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override sifcash.conf
